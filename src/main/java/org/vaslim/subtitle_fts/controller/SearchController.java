...
public class SearchController {
    ...
    @GetMapping("/fuzzy")
    public ResponseEntity<List<MediaRecordDTO>> getVideoByQueryFuzzy(@RequestParam(name = "maxResults", defaultValue = "90") Integer maxResults, @RequestParam String query){
        return ResponseEntity.ok(subtitleService.findVideosByTitleOrSubtitleContentFuzzy(query, maxResults));
    }

    @GetMapping("/exact")
    public ResponseEntity<List<MediaRecordDTO>> getVideoByQueryExact(@RequestParam(name = "maxResults", defaultValue = "90") Integer maxResults,@RequestParam(name = "query") String query, @RequestParam("categoryInfo") String categoryInfo){
